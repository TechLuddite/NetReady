...
import { Radar, Play, RefreshCw, CheckCircle2, AlertCircle, ShieldAlert, Wifi, Server, Layers, Cpu, Search, Filter, AlertTriangle } from 'lucide-react';
...
export const PortScanner: React.FC<PortScannerProps> = ({ onHistoryUpdate }) => {
  ...
  const [scanProgress, setScanProgress] = useState<{ scanned: number; total: number; currentPort?: number }>({
    scanned: 0,
    total: 0,
  });
    ...
    for (const raw of rawParts) {
      ...
      if (part.includes('-')) {
        ...
        if (!isNaN(start) && !isNaN(end) && start > 0 && end <= 65535 && start <= end) {
          // cap range size to 100 ports per custom scan for browser responsiveness
  ...
  const handleAutoDetectSubnet = async () => {
    ...
    try {
      ...
      if (ipParts.length === 4) {
        ...
        setTargetHost(localIp);
        setDetectedSubnetInfo(`Detected Local Interface: ${localIp} (Subnet: ${subnet}, Gateway: ${gateway})`);
      } else {
  ...
  const executeScan = async () => {
    ...
    setScanProgress({ scanned: 0, total: portsToScan.length });

    try {
      const res = await scanPortList(targetHost, portsToScan, (scanned, total, last) => {
        setScanProgress({ scanned, total, currentPort: last.port });
      });
      ...
      const historyItem: HistoryItem = {
        ...
        summary: `Scanned ${res.scannedPorts.length} ports | Open: ${res.openPortsCount} | Closed: ${res.closedPortsCount} | Filtered: ${res.filteredPortsCount} (${res.scanDurationMs}ms)`,
        data: res,
      };
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-xs text-slate-400 max-w-xl leading-relaxed">
            Scan host IPs, local dev servers, or subnets for open TCP socket services using client-side network probes. Results persist safely in browser LocalStorage.
          </p>
        </div>
        ...
        <button
          ...
          disabled={isScanning}
          className="relative z-10 flex items-center space-x-2 bg-cyan-600 hover:bg-cyan-500 text-white font-semibold px-6 py-3 rounded-xl text-sm transition-all shadow-[0_0_20px_rgba(8,145,178,0.3)] active:scale-95 disabled:opacity-50"
        >
          {isScanning ? (
            <>
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center space-x-2">
              ...
              <span>Target Host, IP Address, or Subnet</span>
            </label>

            <button
              onClick={handleAutoDetectSubnet}
              disabled={detectingSubnet || isScanning}
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-800 border border-slate-700 text-xs font-mono text-cyan-300 transition-colors w-fit"
            >
              {detectingSubnet ? (
              ...
              <span>Auto-Detect Subnet / Local Interface</span>
            </button>
          </div>
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="e.g. 127.0.0.1, localhost, 192.168.1.1, or scanme.nmap.org"
              value={targetHost}
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            {[
              ...
              { label: 'Gateway (10.0.0.1)', value: '10.0.0.1' },
              { label: 'Public Test (scanme.nmap.org)', value: 'scanme.nmap.org' },
            ].map((p) => (
              <button
                key={p.value}
                onClick={() => setTargetHost(p.value)}
                className="px-2.5 py-1 bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 rounded-lg text-xs font-mono text-slate-300 transition-colors"
              >
                {p.label}
              </button>
            ))}
        ...
        <div className="bg-white/[0.03] border border-cyan-500/30 rounded-2xl p-6 shadow-xl space-y-3">
          <div className="flex items-center justify-between text-xs font-mono">
            <span className="text-cyan-300 font-bold flex items-center space-x-2">
              ...
              <span>Scanning host {targetHost}...</span>
            </span>
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between border-b border-white/5 pb-6 gap-6">
            <div>
              ...
              <div className="text-xs text-slate-500 font-mono mt-1">
                Completed in {activeResult.scanDurationMs} ms &bull; {new Date(activeResult.timestamp).toLocaleString()}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 font-mono text-center w-full lg:w-auto">
              ...
              <div className="bg-slate-800/60 border border-slate-700/60 px-5 py-3 rounded-xl">
              ...
              <div className="bg-amber-500/10 border border-amber-500/30 px-5 py-3 rounded-xl">
            ...
            <div className="flex items-center space-x-1.5 bg-slate-950 p-1 rounded-xl border border-white/10 text-xs font-mono w-full sm:w-auto">
              {(['all', 'open', 'closed', 'filtered'] as const).map((status) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg uppercase transition-all font-semibold ${
                    filterStatus === status
                ...
                {filteredPorts.length === 0 ? (
                  ...
                  filteredPorts.map((p) => (
                    <tr key={p.port} className="hover:bg-white/[0.02] transition-colors">
                      <td className="py-3 px-4 font-mono font-medium text-cyan-400">{activeResult.targetHost}</td>
